...
class MoneyDepositSerializer(serializers.HyperlinkedModelSerializer):
    records = serializers.HyperlinkedRelatedField(
        many=True, read_only=True, view_name="record-detail"
    )
    ...
    def get_amount(self, obj):
        values = Record.objects.filter(money_deposit=obj).aggregate(
            income=Sum(Case(When(recordType="Income", then="amount"))),
            expense=Sum(Case(When(recordType="Expense", then="amount"))),
        )
